...
# CONFIG
# =============================
BOT_TOKEN = os.getenv("BOT_TOKEN")  # Telegram bot token (set in Railway/Render)
SYMBOLS = ["OPUSDT", "RENDERUSDT", "BTCUSDT"]
TIMEFRAMES = ["5m", "15m"]
HISTORICAL_CANDLES = 200
...
# =============================
# INITIAL ALERTS ON DEPLOYMENT
# =============================
async def initial_alerts():
    for symbol in SYMBOLS:
        # 1H close
        try:
            url = f"https://api.binance.com/api/v3/klines?symbol={symbol}&interval=1h&limit=2"
            ...
            print(f"Error fetching 1H close for {symbol}: {e}")

        # EMA cross for 5m & 15m
        for interval in TIMEFRAMES:
            try:
                ...
                print(f"Error checking initial EMA cross for {symbol} ({interval}): {e}")
