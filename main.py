...
# LEVERAGE = 20
# TARGET_ACCOUNT_PROFIT_PCT = 10.0  # take-profit target
# MAX_CANDLES_TO_CHECK = 5
# MAX_HISTORY = 10
# PLOT_UPDATE_INTERVAL = 1800  # 30 min
...
#         return None
#     X = [d["features"] for d in data if None not in d["features"]]
#     # ✅ Only consider signals with profit >= 5% as "successful"
#     y = [1 if d["profit"] >= 5 else 0 for d in data]
#     if len(X) < 30:
#         return None
#     model = xgb.XGBClassifier(eval_metric="logloss")
#     model.fit(np.array(X), np.array(y))
...
#         avg_profit = np.mean(signal_history[symbol]) if signal_history[symbol] else 0
#         dynamic_threshold = 65 + min(max(avg_profit, 0), 15)
...
# ---------------- LIVE LEARNING ----------------
def record_live_example(symbol, closes, volumes, profit):
    if profit < -5 or abs(profit) < 3:
        return  # skip uninformative samples
    feats = compute_features(closes, volumes)
    if None in feats:
        return
    data = []
...
def retrain_from_live_data(symbol):
    ...
    if len(data) < 30:
        return None
    X = [d["features"] for d in data if None not in d["features"]]
    y = [1 if d["profit"] >= 10 else 0 for d in data]  # only learn 10%+ profitable
    if len(X) < 30:
        return None
    model = xgb.XGBClassifier(
        eval_metric="logloss",
...
async def monitor_ema(symbol, interval):
    ...
    while True:
        ...
        avg_profit = np.mean(signal_history[symbol]) if signal_history[symbol] else 0
        base_threshold = 75
        dynamic_threshold = base_threshold + min(max(avg_profit, 0), 10)
